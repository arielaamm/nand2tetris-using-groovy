public class Tokenizing {
    def tokenizing(File folder)
    {
        folder.eachFile { file ->
            ...
            {
                def outputFile = new File("${folder.path}/${file.name-".jack"}TNew.xml").newWriter()
                String buffer = ""
                buffer += "<tokens>\n"
    ...
    def String writeToXml(File file) {
        ...
        file.eachLine { line ->
            if (!line.trim().startsWith("//") && !line.trim().startsWith("/**") && !line.trim().startsWith("*")) {
                line = extractLinesBeforeComments(line)
    ...
    def extractLinesBeforeComments(String content) {
